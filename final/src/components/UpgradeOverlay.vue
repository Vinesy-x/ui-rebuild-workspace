<!--
  ...
  出现帧: 0014 亲疏度升级
  全屏粉心 ✨ + 段位大字 + 1-2 秒自动消失
  motif 可换: heart | crown | star
-->
<script setup lang="ts">
import { onMounted } from 'vue'
import { useModalStore } from '../stores/useModalStore'
interface Props {
  motif?: 'heart' | 'crown' | 'star'
  label: string         // "熟人" / "朋友" / "正主"
  autoMs?: number
}
...
onMounted(() => {
  setTimeout(() => store.close(), props.autoMs)
})
</script>
...
.heart { display:block; width:120px; height:120px;
  background: #FF6B8C;
  clip-path: polygon(50% 100%, 0 38%, 25% 0, 50% 25%, 75% 0, 100% 38%);
  filter: drop-shadow(0 0 24px #FF6B8C);
}
.label { font-family: var(--font-display); font-size: 56px;
  color: var(--paper-1); letter-spacing: .2em; margin-top: 24px;
